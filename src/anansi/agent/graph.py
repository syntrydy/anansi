"""
LangGraph pipeline: N1 → N2 → N3 → (N4 ∥ N6) → N5.
"""
...
def node_scriptor(state: AnansiState) -> dict[str, Any]:
    ...
    context = CountryData(**state["context_pack"])
...
def build_graph() -> Any:
    """Compile the Anansi teaching pipeline graph."""
...
async def run_pipeline(initial: AnansiState) -> AnansiState:
    """Run end-to-end async pipeline; returns final state including `package`."""
    graph = build_graph()
